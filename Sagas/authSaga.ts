...
async function getUserWithAPI(email: { payload: string | number | boolean }) {
  try {
    ...
    let response = await fetch(
      `${process.env.REACT_APP_URL}/user/email?address=${encodeURIComponent(
        email.payload
      )}`,
      init
    );
...
const signInWithApi = async (email: any, password: any) => {
  try {
...
export function* signIn({ payload: { email, password } }) {
  try {
    const response = yield call(signInWithApi, email, password);
...
export function* authSagas() {
  yield all([
    ...
    yield takeLatest(
      AuthActions.CONFIRM_FORGOT_PASSWORD_REQUEST,
